...
class Calculator::RussianPost < Calculator
  ...
  def compute(line_items=nil)
    total_weight = line_items.map { |li| li.variant.weight.nil? ? DEFAULT_PRODUCT_WEIGHT : li.variant.weight}.sum
    delivery_zone = get_delivery_zone line_items.last.order.checkout.ship_address.zipcode
    get_price delivery_zone, total_weight
  end
