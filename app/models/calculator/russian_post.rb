...
class Calculator::RussianPost < Calculator
  ...
  def compute(line_items=nil)
    ...
    delivery_price = get_delivery_price(delivery_zone, total_weight)
    delivery_price + get_insurance_fee(order)
  end
  ...
  def get_total_weight (line_items)
    line_items.map { |li| li.variant.weight.nil? ? DEFAULT_PRODUCT_WEIGHT : li.variant.weight}.sum
  end

  def get_delivery_zone (zipcode)
    INDEX_TO_ZONE_HASH[zipcode.to_s[0,3]]
  end
  ...
  def get_insurance_fee (order)
    order.item_total * POST_INSURANCE_FEE/100
  end
